...
#ifndef METAVISION_SDK_ANALYTICS_PERIOD_MAP_ASYNC_ALGORITHM_H
#define METAVISION_SDK_ANALYTICS_PERIOD_MAP_ASYNC_ALGORITHM_H
...
class PeriodMapAsyncAlgorithmInternal;

/// @brief Class that estimates the pixel-wise period of vibrating objects using Metavision Vibration API
class PeriodMapAsyncAlgorithm {
public:
    ...
    /// @brief Builds a new @ref PeriodMapAsyncAlgorithm object
    /// @param width Sensor's width (in pixels)
    /// @param height Sensor's height (in pixels)
    /// @param period_map_async_config Period estimation configuration
    PeriodMapAsyncAlgorithm(int width, int height, const PeriodEstimationConfig &period_map_async_config);

    /// Default destructor
    ~PeriodMapAsyncAlgorithm();

    /// @brief Sets a callback to get the output period map
    /// @param output_cb Callback to call
    void set_output_callback(const OutputCb &output_cb);

    /// @brief Sets the frequency at which the algorithm generates the period map
    /// @param freq Frequency at which the period map will be generated
...
private:
    std::unique_ptr<PeriodMapAsyncAlgorithmInternal> algo_;
};

} // namespace Metavision

#endif // METAVISION_SDK_ANALYTICS_PERIOD_MAP_ASYNC_ALGORITHM_H
